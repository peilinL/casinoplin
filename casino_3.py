...
def play_dice(parts):
    global balance, bet
    if len(parts) != 4:
        print("Usage: .dice [bet amount] [under/over] [target number]")
        print("Example: .dice 100 under 50")
        return
    ...
    if direction not in ("under", "over"):
        print("Choose under or over.")
        return

    try:
        target = float(parts[3])
    except ValueError:
        print("Please enter a number target.")
        return
    ...
    if winchance < 1 or winchance > 98:
        print("Target must give a win chance from 1% to 98%.")
        return
    ...
    print("Dice roll:", money(roll))
    print("Mode: roll",direction,money(target))
    print("Win chance:",money(winchance)+"%")
    print("Multiplier:",money(multiplier)+"x")

    if won:
        balance = round(balance + payout, 2)
        print("You win! Profit:",money(profit))
    else:
        print("You lose. Profit: -"+money(bet))
    print("New balance:",money(balance))
...
while True:
    ...
    if len(parts) == 0:
        ...
        print(".cf [betamount] [heads/tails]: play coinflip with bet amount, bet heads/tails.")
        print(".dice [betamount] [under/over] [target]: play dice with a 1% house edge.")
        print(".quit: leave the casino.")
    #devcommands sneaky sneaky
    elif parts[0] == ".addbal" and len(parts) == 2 and parts[1].isdigit():
        balance = balance + int(parts[1])
        print("Added",parts[1],"dollars.")
        print("New balance:",balance)
    elif parts[0] == ".addbal":
        print("Please enter integer amount.")
    elif parts[0] == ".removebal" and len(parts) == 2 and parts[1].isdigit():
        balance = balance - int(parts[1])
        print("Removed",parts[1],"dollars.")
        print("New balance:",balance)
    elif parts[0] == ".removebal":
        print("Please enter integer amount.")

    elif parts[0] == ".bal":
        print("Your balance is:",money(balance))
    elif parts[0] == ".dice":
        play_dice(parts)
    elif parts[0] == ".cf" and len(parts) != 3:
        print("Please enter bet and heads/tails.")
        continue
    elif parts[0] == ".cf" and len(parts) == 3 and not parts[1].isdigit():
        print("Please enter integer bet.")
        continue
    elif parts[0] == ".cf" and len(parts) == 3 and parts[1].isdigit() and parts[2] in ("heads","tails"):
        bet = int(parts[1])
        if bet > balance:
            print("Insufficient balance, bet set to maximum value")
            bet = balance
        ...
        if bet < 1:
            print("Bet must be at least 1.")
            continue
        print("Game started, bet is",str(bet)+".")
        game = True
        balance = balance - bet
        while game == True:
            result = random.randint(0,1)
            if result == 0 and parts[2] == "heads":
                print("Heads! You win!")
                balance = balance + 2*bet
                print("New balance:",balance)
            elif result == 1 and parts[2] == "tails":
                print("Tails! You win!")
                balance = balance + 2*bet
                print("New balance:",balance)
            elif result == 0 and parts[2] == "tails":
                print("Heads! You lose!")
                print("New balance:",balance)
            elif result == 1 and parts[2] == "heads":
...
#continuing to lobby commands
        ...
        print("Game not started yet.")
    elif parts[0] not in [".help", ".quit", ".bal", ".bj", ".hit", ".stand",".double",".split",".cf",".dice"]:
        continue
#continue game    
